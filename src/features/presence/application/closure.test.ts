...
describe("activity day closure", () => {
  ...
  it("moves an unpicked-up child into garderie once the activity is closed", async () => {
    ...
    await markStillPresent("activity-velo", "child-29", "monitor-3");

    const beforeClose = await getActivityDetail("activity-velo");
    const child = beforeClose?.eveningList.find((c) => c.childId === "child-29");
    expect(child?.status).not.toBe("DAYCARE");

    await closeActivityDay("activity-velo", "monitor-3", "Moniteur 3");

    const afterClose = await getActivityDetail("activity-velo");
